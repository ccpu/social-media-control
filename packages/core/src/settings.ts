...
export class Settings implements SettingsData {
  ...
  // If enabled, move to the next reel after playback ends (Instagram /reels, Facebook /reel).
  public get advanceOnVideoEnd(): boolean {
    return this.data.advanceOnVideoEnd;
  }

  public set advanceOnVideoEnd(value: boolean) {
    if (this.data.advanceOnVideoEnd === value) return;
    this.data.advanceOnVideoEnd = value;

    this.onChange('advanceOnVideoEnd');
  }
  ...
  private storeValues(data: { [p: string]: unknown }) {
    ...
    if (typeof data.advanceOnVideoEnd === 'boolean') {
      this.advanceOnVideoEnd = data.advanceOnVideoEnd;
    }
  }

  public applyChanges(data: Partial<SettingsData>): void {
    this.storeValues(data);
  }
