import type { ReactFiber } from './reactFiber';
...
export class ReactHelper {
  ...
  public static getFiberFromDocument(): ReactFiber | undefined {
    ...
    for (const node of document.body.childNodes) {
      const key = Object.keys(node).find((p) => p.startsWith('__reactContainer'));
      if (!key) continue;
   ...
   */
  public static getNodeFromFiber(fiber?: ReactFiber): HTMLElement | undefined {
  ...
  public static traversalParents<T>(
    ...
    if (fiber.tag === ReactTag.HostPortal) {
      ...
      if (portalNode && portalNode.parentElement) {
        const parentFiber = this.getFiberFromNode(portalNode.parentElement);
    ...
    return this.traversalChildren(root, (fiber) => {
      ...
      if (
        ...
        (names.includes(className) ||
          lookupNames.some((lookupName) => className.endsWith(lookupName)))
      )
  ...
  public static getPortalNode(fiber: ReactFiber): HTMLElement | undefined {
    if (fiber.tag !== ReactTag.HostPortal) return undefined;
