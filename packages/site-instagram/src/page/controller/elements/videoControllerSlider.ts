...
export abstract class VideoControllerSlider extends VideoControllerElementBase<HTMLDivElement> {
  ...
  override onCreate(parentElement: HTMLElement): void {
    ...
    this.element.classList.add('smc-control-element', 'smc-control-bar');
    parentElement.appendChild(this.element);

    this.backgroundElement = document.createElement('div');
    this.backgroundElement.classList.add('smc-control-bar-background');
    this.element.appendChild(this.backgroundElement);

    this.progressElement = document.createElement('div');
    this.progressElement.classList.add('smc-control-bar-progress');
    ...
    this.element.addEventListener('mousedown', (ev) => this.onDragStart(ev));
    this.element.addEventListener('touchstart', (ev) => this.onDragStart(ev));
