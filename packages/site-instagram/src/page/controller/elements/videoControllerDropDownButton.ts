...
> extends VideoControllerButton {
  ...
  override onCreate(parentElement: HTMLElement): void {
    ...
    this.dropDownElement.classList.add('smc-control-dropdown', 'hidden');
  ...
  protected addItem(value: T, label: string): void {
    if (!this.dropDownElement) return;
  ...
  protected setSelectedItem(selectedItem: T | undefined): void {
  ...
  override onClick(): void {
    // The drop-down opens on hover, clicks are ignored.
  }

  private dropDownDelayTimeout: number = -1;

  // Sets the drop-down visibility.
  protected setDropDownVisibility(visibility: boolean): void {
    if (!this.dropDownElement) return;
    ...
    if (visibility) {
      ...
      this.dropDownDelayTimeout = window.setTimeout(() => {
        if (!this.dropDownElement) return;
